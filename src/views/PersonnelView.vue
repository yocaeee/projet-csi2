<template>
        ...
        <v-card>
          ...
          <v-card-text>
            <v-form>
              ...
              <v-text-field v-model="nouveauPersonnel.motDePasse" label="Mot de passe" type="password" required></v-text-field>
              <v-select
                v-model="nouveauPersonnel.role"
                :items="['Administrateur', 'Moniteur', 'Propriétaire', 'Garçon de Plage']"
                label="Rôle"
                required
              ></v-select>
            </v-form>
  ...
  console.log("Objet reçu dans confirmDelete:", person);

  // Vérifier quelle clé est présente et corriger si nécessaire
  ...
  personnelToDelete.value = { ...person, idutilisateur: userId }; // Stocker avec la bonne clé
